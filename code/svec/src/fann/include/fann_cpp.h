	...
	*/
	void copy_from_struct_fann(struct fann* other)
	{
        ...
        */ 
		...
		bool create_from_file_with_weight_factors(const std::string &configuration_file, bool with_weight_factors) {
			if (create_from_file(configuration_file)) {
				
				unsigned int total = get_total_connections();
				FANN::connection *con = new FANN::connection[total];								
				get_connection_array(con);
				
				if (with_weight_factors) {
					std::ifstream ifs;
					ifs.open((configuration_file+"wf").c_str());								
					for (unsigned int idx = 0; idx<total; idx++) {
						ifs >> con[idx].from_neuron >> con[idx].to_neuron >> con[idx].weight_factor;					
					}
					ifs.close();									
				} else {
					for (unsigned int idx = 0; idx<total; idx++) {
						con[idx].weight_factor = 1;
					}
				}
				set_weight_factor_array(con, total);
				return true;
			}
			return false;		
		}
		...
		bool save_with_weight_factors(const std::string &configuration_file)
        {
            if (save(configuration_file))
            {				
				unsigned int total = get_total_connections();
				FANN::connection *con = new FANN::connection[total];								
				get_connection_array(con);
				std::ofstream ofs;
				ofs.open((configuration_file+"wf").c_str());
				for (unsigned int idx = 0; idx<total; idx++) {									
					ofs << con[idx].from_neuron << " " << con[idx].to_neuron << " " << con[idx].weight_factor << "\n";										
				}
				ofs.close();				
            }            
            return true;
        ...
        */ 
		...
		/*
		 marcel
		*/
		void get_last_activations(unsigned int layer_index, double *activations) {
			struct fann_layer *layer_it;
			struct fann_neuron *neuron_it;			

			if (ann != NULL) {
				layer_it = ann->first_layer + layer_index;
				/*for (unsigned int i=0; i<layer_index; i++) {
					layer_it++;
				}*/
				int i = 0;
				for (neuron_it = layer_it->first_neuron; neuron_it != layer_it->last_neuron; neuron_it++, i++) {
					activations[i] = neuron_it->value;					
				}				
			}			
		}
		...
		/*
		 marcel
		*/
		void get_last_sums(unsigned int layer_index, double *sums) {
			struct fann_layer *layer;
			struct fann_neuron *neuron_it;			

			if (ann != NULL) {				
				layer = ann->first_layer + layer_index;

				int i = 0;
				for (neuron_it = layer->first_neuron; neuron_it != layer->last_neuron; neuron_it++, i++) {
					sums[i] = neuron_it->sum;					
				}
			}			
		}


		std::string print_parameters2() {
			struct fann_layer *layer_it;
			unsigned int i;
			...
			if (ann != NULL) {
				...
				sprintf(dest, "Input layer                          :%4d neurons, 1 bias\n", ann->num_input);
				s += dest;
				for(layer_it = ann->first_layer + 1; layer_it != ann->last_layer - 1; layer_it++)
				{
					if(ann->network_type == FANN_NETTYPE_SHORTCUT)
					{
						sprintf(dest,"  Hidden layer                       :%4d neurons, 0 bias\n",
							   (int)(layer_it->last_neuron - layer_it->first_neuron));
						s += dest;
					}
					else
					{
						sprintf(dest,"  Hidden layer                       :%4d neurons, 1 bias\n",
							   (int)(layer_it->last_neuron - layer_it->first_neuron - 1));
						s += dest;
					}
				}
				sprintf(dest, "Output layer                         :%4d neurons\n", ann->num_output); s += dest;
				sprintf(dest, "Total neurons and biases             :%4d\n", fann_get_total_neurons(ann)); s += dest;
				sprintf(dest, "Total connections                    :%4d\n", ann->total_connections); s += dest;
				sprintf(dest, "Connection rate                      :%8.3f\n", ann->connection_rate); s += dest;
				sprintf(dest, "Network type                         :   %s\n", FANN_NETTYPE_NAMES[ann->network_type]); s += dest;
			
				sprintf(dest, "Training algorithm                   :   %s\n", FANN_TRAIN_NAMES[ann->training_algorithm]); s += dest;
				sprintf(dest, "Training error function              :   %s\n", FANN_ERRORFUNC_NAMES[ann->train_error_function]); s += dest;
				sprintf(dest, "Training stop function               :   %s\n", FANN_STOPFUNC_NAMES[ann->train_stop_function]); s += dest;
						
				sprintf(dest, "Bit fail limit                       :%8.3f\n", ann->bit_fail_limit); s += dest;
				sprintf(dest, "Learning rate                        :%8.3f\n", ann->learning_rate); s += dest;
				sprintf(dest, "Learning momentum                    :%8.3f\n", ann->learning_momentum); s += dest;
				sprintf(dest, "Quickprop decay                      :%11.6f\n", ann->quickprop_decay); s += dest;
				sprintf(dest, "Quickprop mu                         :%8.3f\n", ann->quickprop_mu); s += dest;
				sprintf(dest, "RPROP increase factor                :%8.3f\n", ann->rprop_increase_factor); s += dest;
				sprintf(dest, "RPROP decrease factor                :%8.3f\n", ann->rprop_decrease_factor); s += dest;
				sprintf(dest, "RPROP delta min                      :%8.3f\n", ann->rprop_delta_min); s += dest;
				sprintf(dest, "RPROP delta max                      :%8.3f\n", ann->rprop_delta_max); s += dest;
				sprintf(dest, "Cascade output change fraction       :%11.6f\n", ann->cascade_output_change_fraction); s += dest;
				sprintf(dest, "Cascade candidate change fraction    :%11.6f\n", ann->cascade_candidate_change_fraction); s += dest;
				sprintf(dest, "Cascade output stagnation epochs     :%4d\n", ann->cascade_output_stagnation_epochs); s += dest;
				sprintf(dest, "Cascade candidate stagnation epochs  :%4d\n", ann->cascade_candidate_stagnation_epochs); s += dest;
				sprintf(dest, "Cascade max output epochs            :%4d\n", ann->cascade_max_out_epochs); s += dest;
				sprintf(dest, "Cascade min output epochs            :%4d\n", ann->cascade_min_out_epochs); s += dest;
				sprintf(dest, "Cascade max candidate epochs         :%4d\n", ann->cascade_max_cand_epochs); s += dest;
				sprintf(dest, "Cascade min candidate epochs         :%4d\n", ann->cascade_min_cand_epochs); s += dest;
				sprintf(dest, "Cascade weight multiplier            :%8.3f\n", ann->cascade_weight_multiplier); s += dest;
				sprintf(dest, "Cascade candidate limit              :%8.3f\n", ann->cascade_candidate_limit); s += dest;
				for(i = 0; i < ann->cascade_activation_functions_count; i++)
					sprintf(dest,"Cascade activation functions[%d]      :   %s\n", i,
						FANN_ACTIVATIONFUNC_NAMES[ann->cascade_activation_functions[i]]); s += dest;
				for(i = 0; i < ann->cascade_activation_steepnesses_count; i++)
					sprintf(dest,"Cascade activation steepnesses[%d]    :%8.3f\n", i,
						ann->cascade_activation_steepnesses[i]); s += dest;
		
				sprintf(dest,"Cascade candidate groups             :%4d\n", ann->cascade_num_candidate_groups); s += dest;
				sprintf(dest,"Cascade no. of candidates            :%4d\n", fann_get_cascade_num_candidates(ann)); s += dest;
	...
	/* Method: get_sarprop_temperature
        ...
        */
		...
		/* marcel */
		void set_weight_factor_array(connection *connections, unsigned int num_connections)
		{
			if (ann != NULL) 
			{
				fann_set_weight_factor_array(ann, connections, num_connections);
			}
		}
