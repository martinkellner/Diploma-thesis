		...
		for(layer_it = ann->first_layer + 1; layer_it != last_layer; layer_it++)
		{
			...
			for(neuron_it = layer_it->first_neuron; neuron_it != last_neuron; neuron_it++)
			{
				...
				for(i = neuron_it->first_con; i != tmp_con; i++)
				{
					...
					/* marcel */
					ann->weight_factors[i] = (fann_type) 1.0;
				...
				ann->weight_factors[tmp_con] = (fann_type) 1.0;
	...
	else
	{
		...
		for(layer_it = ann->first_layer + 1; layer_it != ann->last_layer; layer_it++)
		{
			...
			for(neuron_it = layer_it->first_neuron; neuron_it != last_neuron; neuron_it++)
			{
				...
				ann->weight_factors[neuron_it->first_con] = (fann_type) 1.0;
			}
...
FANN_EXTERNAL fann_type *FANN_API fann_run(struct fann * ann, fann_type * input)
{
	...
	fann_type *weights, *weight_factors;
	...
	for(layer_it = ann->first_layer + 1; layer_it != last_layer; layer_it++)
	{
		...
		for(neuron_it = layer_it->first_neuron; neuron_it != last_neuron; neuron_it++)
		{
			...
			weight_factors = ann->weight_factors + neuron_it->first_con;
			...
			if(ann->connection_rate >= 1)
			{
				...
				*/
				...
				else
				{
					...
					for(i = 0;i != num_connections; i++){
					//printf("%f += %f*%f, ", neuron_sum, weights[i], neurons[i].value);
					 ww = fann_mult(weights[i], weight_factors[i]);					
					 neuron_sum += fann_mult(ww, neurons[i].value);
					}
				}
...
FANN_EXTERNAL void FANN_API fann_destroy(struct fann *ann)
{
	...
	fann_safe_free(ann->weight_factors);
    ...
    for(layer_it = ann->first_layer; layer_it != ann->last_layer; layer_it++){
        ...
        for(neuron_it = layer_it->first_neuron; neuron_it != layer_it->last_neuron; neuron_it++){
            ...
            for (idx = neuron_it->first_con; idx < neuron_it->last_con; idx++){
				...
				/* marcel */
				connections->weight_factor = ann->weight_factors[source_index];
...
FANN_EXTERNAL void FANN_API fann_set_weight_factor_array(struct fann *ann,
    struct fann_connection *connections, unsigned int num_connections)
{
    unsigned int idx;

	fann_type *weight_factors = ann->weight_factors;

	
	for (idx = 0; idx < num_connections; idx++) {
		weight_factors[idx] = connections[idx].weight_factor;
		/*
		fann_set_weight_factor(ann, connections[idx].from_neuron,
			connections[idx].to_neuron, connections[idx].weight_factor);
			*/
    }
}
...
FANN_EXTERNAL void FANN_API fann_set_weight_factor(struct fann *ann,
    unsigned int from_neuron, unsigned int to_neuron, fann_type weight_factor)
{
    struct fann_neuron *first_neuron;
    struct fann_layer *layer_it;
    struct fann_neuron *neuron_it;
        ...
        for(neuron_it = layer_it->first_neuron; neuron_it != layer_it->last_neuron; neuron_it++){
            ...
            for (idx = neuron_it->first_con; idx < neuron_it->last_con; idx++){
                ...
                if (((int)from_neuron == ann->connections[source_index] - first_neuron) &&
                    (to_neuron == destination_index))
                {
                    ann->weight_factors[source_index] = weight_factor;
                }
                source_index++;
            }
            destination_index++;
...
void fann_allocate_connections(struct fann *ann)
{
	...
	/* marcel */
	ann->weight_factors = (fann_type *) calloc(ann->total_connections, sizeof(fann_type));
	if(ann->weight_factors == NULL)
	{
		fann_error((struct fann_error *) ann, FANN_E_CANT_ALLOCATE_MEM);
		return;
	}
